...
class NJPermitsIngest:
    def __init__(self):
        ...
        self.api_url = "https://data.nj.gov/resource/ipay-4j5v.json"
        ...
        for permit in permits:
            try:
                ...
                processed_permit = {
                    ...
                    'estimated_value': float(permit.get('estimated_value', 0)) if permit.get('estimated_value') else 0,
    ...
    def run(self):
        ...
        try:
            ...
            if not permits:
                print("⚠️ No permits fetched. Exiting.")
                return
            
            # Process permits
            processed_permits = self.process_permits(permits)
            if not processed_permits:
                print("⚠️ No valid permits to process. Exiting.")
                return
            
            # Load to database
            self.load_to_database(processed_permits)
            
            print("🎉 NJ Permits ingestion completed successfully!")
            
        except Exception as e:
            print(f"💥 NJ Permits ingestion failed: {e}")
            sys.exit(1)

if __name__ == "__main__":
    ingester = NJPermitsIngest()
    ingester.run()
